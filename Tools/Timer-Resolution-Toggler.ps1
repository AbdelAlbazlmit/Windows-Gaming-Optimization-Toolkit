...
switch ($choice) {
    "1" {
        ...
        Write-Host "[✓] Timer resolution set to 0.5ms" -ForegroundColor Green
    }
    "2" {
        bcdedit /set useplatformclock no
        bcdedit /set tscsyncpolicy default
        Write-Host "[✓] Timer resolution set to 1.0ms" -ForegroundColor Green
    }
    "3" {
        bcdedit /deletevalue useplatformclock
        bcdedit /deletevalue tscsyncpolicy
        bcdedit /deletevalue disabledynamictick
        Write-Host "[✓] Timer resolution reset" -ForegroundColor Green
    }
}
